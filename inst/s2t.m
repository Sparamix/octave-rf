...
## @verbatim
##   T11 = -(S11*S22 - S12*S21) / S21 = -det(S) / S21
##   T12 =  S11 / S21
##   T21 = -S22 / S21
##   T22 =  1   / S21
## @end verbatim
...
##     Section 3.6 "T-Parameters", Eq. 3.21 (p.65) defines T:
##       (b1, a1)^T = T * (a2, b2)^T.
##     Section 3.6.1, Eq. 3.31-3.32 (p.68): T in terms of S — matches
...
##     Section 9.2.4 "Cascading S-Parameters" (p.390) — T-matrix cascade
##     as the practical motivation for this conversion.
##
...
##   plane shifts, which are a different concept.  No Pozar citation here.
## @end verbatim
##
...
function T = s2t (S)
  ...
  for k = 1:K
    ...
    T(1,1,k) = -(S(1,1,k)*S(2,2,k) - S(1,2,k)*S(2,1,k)) / s21;
    T(1,2,k) =  S(1,1,k) / s21;
    T(2,1,k) = -S(2,2,k) / s21;
    T(2,2,k) =  1.0       / s21;
  endfor
...
%! %% Expected T: T11=1, T12=1/3, T21=-1/3, T22=3/2
%! S = reshape([1/3, 2/3, 2/3, 1/3], 2, 2, 1);
%! T = s2t(S);
%! %% Verify T22 = 1/S21 = 3/2
%! assert (T(2,2,1), 1/(2/3), 1e-14);
%! %% Verify T12 = S11/S21 = (1/3)/(2/3) = 1/2
%! assert (T(1,2,1), (1/3)/(2/3), 1e-14);
%! %% Verify T21 = -S22/S21 = -1/2
%! assert (T(2,1,1), -(1/3)/(2/3), 1e-14);
%! %% Verify T11 = -det(S)/S21; det(S)=(1/3)^2-(2/3)^2 = 1/9-4/9 = -3/9 = -1/3
%! %% T11 = -(-1/3)/(2/3) = (1/3)/(2/3) = 1/2
%! assert (T(1,1,1), -((1/3)*(1/3) - (2/3)*(2/3))/(2/3), 1e-14);
...
%! T = s2t(S);
%! %% det(S) = S11*S22 - S12*S21; T11 = -det(S)/S21
%! det_S = S(1,1,1)*S(2,2,1) - S(1,2,1)*S(2,1,1);
%! assert (T(1,1,1), -det_S / S(2,1,1), 1e-14);
